...
public class AppointmentActivity extends BaseActivity implements
	...
	private int Lock_Time = 2000;
		...
		//按键触发状态
		//private volatile boolean isboutn = false;
	private volatile boolean isnetinfo = false;
	private volatile long isnetdata = 0;
	private volatile long isbountdata = 0;
	...
	Handler handler = new Handler() {
		public void handleMessage(Message msg) {
			...
			case RECEIVED:
				...
				/***************收到数据当前时间******************/
				Date isnetcurDate   =   new   Date(System.currentTimeMillis());//
				isnetdata = isnetcurDate.getTime();
				
				break;
			case UPDATE_UI:
				if (isLock)
					break;
				long time = ((isbountdata-isnetdata) > 0) ? (isbountdata-isnetdata) : (isnetdata-isbountdata);
				if (time < 1000){
					isnetinfo = true;
					break;
				}
	...
	public void onCheckedChanged(CompoundButton arg0, boolean isCheck) {
		Date isbountcurDate = new Date(System.currentTimeMillis());
		isbountdata = isbountcurDate.getTime();
		switch (arg0.getId()) {
...
//			tvDelayTime.setVisibility(View.GONE);
	...
	private int bytes2Integer(ArrayList<Boolean> mList) {
		...
		for (int i = 0; i < 8; i++) {
			if (mList.get(i)) {
				result += (Math.pow(2, i));//Math.pow(x,y);返回底数的指定次幂;即是2的i次方
			}
		}
