...
//				return -1; 
	...
	*/
	...
	class NetworkBlock
	{
		...
		public String getValueByKey(String Key)//ssid pwd
		{
			String Value = "";
			String tmpString = "";
			int iStart = 0;
			...
			for (int i = 0; i < keyValueList.size(); i ++)
			{
				...
				{
					iStart = tmpString.indexOf("=");//返回指定子字符串在此字符串中第一次出现处的索引。
					iLength = tmpString.length();
					Value = tmpString.substring(iStart + 1, iLength);//  返回一个新字符串，它是此字符串的一个子字符串。
					Value = Value.trim();//返回字符串的副本，忽略前导空白和尾部空白
					
					if (Value.startsWith("\"") && Value.endsWith("\""))//测试此字符串是否以指定的前缀开始。
					{
